...
xcrun simctl launch <UDID> jp.hibiki.anywheregp
```
...
- `Audio/GameAudio.swift` — procedural sound effects plus one music asset: an `AVAudioSourceNode` synthesizes a speed-following saw-wave engine plus enveloped sine beeps. Synth state is behind an `OSAllocatedUnfairLock`; the render closure must stay `@Sendable` and capture no MainActor state. Uses the `.ambient` session category (respects the silent switch). BGM loops via `AVAudioPlayer`s: bundled AAC tracks `BGMTitle/BGMTimeAttack/BGMRace/BGMPeerRace/BGMRoomDrive.m4a` (converted with `afconvert` from the Ogg Vorbis originals `docs/bgm_title/time/vs/net/free.ogg` — iOS can't play Vorbis). `ContentView`'s per-frame update calls `setMusic(track:suspended:)` on every screen: `currentMusicTrack` returns `.title` on the title/mode-select screens, the mode's track during countdown+racing, nil otherwise (fade out + rewind; suspension pauses in place). Track switches crossfade (the fade task holds the outgoing player, not `currentPlayer`). Explicit `pauseMusic()` on the backgrounding and AR-interruption paths where frame updates may stop. `Audio/Haptics.swift` maps the same events to `UIFeedbackGenerator`s (iOS only).
