...
- `ContentView.swift` — platform wiring. iOS device: `content.camera = .spatialTracking` + `game.installFloorAnchor()` (floor plane, 0.6 × 0.6 m+). The ready screen offers "コースを置き直す" (`reanchorCourse`: drops the `AnchoringComponent` and re-adds it a beat later so it re-resolves against the floor currently in view) and "90°回転" (`rotateCourse`: rotates `root.orientation`; all game logic lives in root-local space so this is safe). macOS/simulator: fallback ground + `PerspectiveCameraComponent` + `.realityViewCameraControls(.orbit)`. Any scene change must keep both branches working (plus the visionOS branch, which gets neither).
